...
export async function GET(request: Request, { params }: Params) {
  try {
    ...
    const { data, error } = await createServiceClient().from(config.table).select("*").order("created_at");
...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    const { data, error } = await createServiceClient().from(config.table).insert(payload as never).select("*").single();
